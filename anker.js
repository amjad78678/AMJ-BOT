...
premium = ["917306149125@s.whatsapp.net"]
...
async function starts() {
	...
	dms.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `*[ ഞങ്ങളുടെ ഗ്രൂപ്പിലേക്ക് താങ്കളെ വരവേൽക്കുന്നു  ${mdata.subject} ]*\n*═══════════════════*\n@${num.split('@')[0]} *ഞാൻ ഒരു ബോട്ട് ആണ് എന്റെ യജമാനൻ അംജദ് !!!*\n*═══════════════════*\n*എനിക്ക് നിങ്ങളെ പരിചയപ്പെട്ടാൽ കൊള്ളാം എന്നുണ്ട് നിങ്ങൾക്ക് താല്പര്യം ഉണ്ടെങ്കിൽ നിങ്ങളുടെ പേര് കാര്യങ്ങൾ എഴുതുക*\n*═══════════════════*\n*✤ name :*\n*✤ age :*\n*✤ gender :*\n*✤ asal kota :*\n*✤ nama IG :*\n*✤ നിങ്ങൾക്ക് ഇഷ്ടമായോ ? :*\n*✤ ഞങ്ങൾ നിന്നെ വിളിക്കേണ്ടത് ? :*\n*═══════════════════*\n*NOTE : SAVE NUMBER BOT & ADMIN*\n*പൊളിക്കല്ലേ മുത്തേ *`
				...
				teks = ` സത്യം പറഞ്ഞാൽ നല്ല വിഷമം ഉണ്ട് പുള്ളേ @${num.split('@')[0]} ഇനി നമുക്ക് സെറ്റ് ആക്കാം പിന്നെ:v`
				let buff = await getBuffer(ppimg)
				dms.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	dms.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					...
					userB: `*Hallo  പുള്ളേ * ${pushname2} *നീ പറയുന്ന കാര്യം ബോട്ട് കേൾക്കണം എന്നുണ്ടെങ്കിൽ ആദ്യം നീ ഇത് അടിക്കുക  *\n*ഇതാണ് അടിക്കേണ്ടത്  ${prefix}daftar name|age*\n*LIKE ${prefix}daftar anker|17*`,
					admin: '*ഇത് നിനക്കൊന്നും പറ്റില്ല പുള്ളേ അഡ്മിനിനു മാത്രമേ ഉപയോഗിക്കാൻ കഴിയൊള്ളു !*',
			...
			const ownerNumber = ["917306149125@s.whatsapp.net"] // owner number ubah aja
       ...
       if (messagesLink.includes("://chat.whatsapp.com/")){
		...
		case prefix+'daftar':
					dms.updatePresence(from, Presence.composing)
					if (isUser) return reply('എടാ ചെങ്ങായി നീ കുറേ മുമ്പേ രജിസ്റ്റർ ചെയ്തതല്ലേ ബാക്കി ഉള്ളവർക്കും രജിസ്റ്റർ ചെയ്യാൻ കൊടുക്ക്‌ മനുഷ്യൻ അല്ലേ പുള്ളേ')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
         ...
         case prefix+'lirik':
                ...
                //if (!isPublic) return reply(mess.only.publikG)
                    ...
                    dms.sendMessage('917306149125@s.whatsapp.net', options, text, {quoted: mek})
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
				reply(mess.wait)
					if (args.length < 1) return reply('നീ എന്ത് എഴുതിയതിനെ ആണ് നീ എന്തോ ആക്കാൻ പറയുന്നത് മനുഷ്യനെ പറ്റിക്കാണോ പുള്ളേ ?')
...
// only grup fitur anime
              case prefix+'anime':
                ...
                //if (!isPublic) return reply(mess.only.publikG)
                if (isLimit(sender)) return reply(limitend(pushname2))
                if (!isAnime) return reply(' *അത് അനിമേഷൻ മോഡ് ഓൺ ആകിയിട്ടില്ല* ')
                ...
                //if (!isPublic) return reply(mess.only.publikG)
                if (isLimit(sender)) return reply(limitend(pushname2))
                if (!isAnime) return reply(' *അത് അനിമേഷൻ മോഡ് ഈ ബോട്ടിനു ഓൺ ആകിയിട്ടില്ല * ')
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
              ...
              case prefix+'randomkpop':
			   ...
			   reply(mess.wait)
				...
				reply(mess.wait)
					...
					dms.sendMessage(from, buffer, video, {mimetype: 'video/mp4', quoted: mek, caption: 'PWOLIKKEDA MWONUSE'})
					await limitAdd(sender) 
					break 
...
//creator
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					dms.sendMessage(from, buffer, image, {caption: 'KONDU POYI PWOLIKKU', quoted: mek})
					await limitAdd(sender) 
					break 
					case prefix+'leavest':
					if (isBanned) return reply(mess.only.benned)    
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					dms.sendMessage(from, buffer, image, {caption: 'PWOLIKKEDA MWONUSE', quoted: mek})
					await limitAdd(sender) 
					break 
					case prefix+'3d':
					  if (!isUser) return reply(mess.only.userB)
                ...
                tahtaq = `${body.slice(7)}`
                     ...
                     dms.sendMessage(from, buff, image, {quoted: mek, caption: `MANUSHYANALLE PULLE`})
                  await limitAdd(sender) 
                  break  
                case prefix+'glow':
                if (!isUser) return reply(mess.only.userB)
                ...
                tahtaw = `${body.slice(6)}`
                     ...
                     dms.sendMessage(from, buff, image, {quoted: mek, caption: `pewer aayille`})
                  await limitAdd(sender) 
                  break  
                  case prefix+'love':
                if (!isUser) return reply(mess.only.userB)
                ...
                tahtae = `${body.slice(6)}`
                     ...
                     dms.sendMessage(from, buff, image, {quoted: mek, caption: `SET AAYILLE`})
                  await limitAdd(sender) 
                  break  
                 case prefix+'nulis':
                if (!isUser) return reply(mess.only.userB)
                ...
                tahtay = `${body.slice(7)}`
                     ...
                     dms.sendMessage(from, buff, image, {quoted: mek, caption: `MONE PWOLIKKU`})
                  await limitAdd(sender) 
                  break  
                case prefix+'glitch':
                if (!isUser) return reply(mess.only.userB)
			    ...
			    tahtar2 = tahtar3.split("/")[1];
					...
					if (args.length < 1) return reply(preff)
                     ...
                     dms.sendMessage(from, buff, image, {quoted: mek, caption: `PWOLIKKEDA MWONUSE`})
                  await limitAdd(sender) 
                  break  
                case prefix+'cantik':
					limitAdd(sender) 
		    ...
		    case prefix+'map':
				...
				if (!isGroupAdmins) return reply(mess.only.admin)
					...
					if ((args[0]) === 'on') {
						...
						reply ('ഇനി പറ്റില്ല ഒരു വട്ടം സെറ്റ് ആയതല്ലേ എന്താണ് പുള്ളേ')
					}
					break
			    case prefix+'nsfw':
			    if (isBanned) return reply(mess.only.benned)    
				...
				//if (!isPublic) return reply(mess.only.publikG)
					...
					if ((args[0]) === 'on') {
						...
						reply(`\`\`\`✓SUCCESS  നീ ഇപ്പൊ പറഞ്ഞ കാര്യം അന്തസ്സ് സെറ്റ് ആക്കിയിട്ടുണ്ട് മുകേഷിനെ വിളിക്കെണോ പുള്ളേ \`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === 'off') {
				...
				if (!isGroup) return reply(mess.only.group)
					...
					if ((args[0]) === 'on') {
						...
						reply(`\`\`\`✓Sukses ANTI LINK GROUP SET AAYI PEWERESH\`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === 'off') {
...
// premium user
       ...
       case prefix+'ytmp4':
					...
					dms.sendMessage(from, buffer, video, {mimetype: 'video/mp4', filename: `${anu.result.title}.mp4`, quoted: mek, caption: 'PWOLIKKEDA MWONUSE'})
					await limitAdd(sender) 
					break 
				...
				case prefix+'ytmp3':
					...
					dms.sendMessage(from, buffer, audio, {mimetype: 'audio/mp4', filename: `${anu.result.title}.mp3`, quoted: mek, caption: 'PWOLIKKEDA MWONUSE'})
					await limitAdd(sender) 
					break 
           ...
           case prefix+'play':
			...
			case prefix+'instagram':
					...
					dms.sendMessage(from, buffer, video, {mimetype: 'video/mp4', filename: `${anu.result.caption}.mp4`, quoted: mek, caption: 'PWOLIKKEDA MWONUSE'})
					await limitAdd(sender) 
					break 
            case prefix+'smule':
	       if (isBanned) return reply(mess.only.benned)
	       //if (!isPrem) return reply(mess.only.premium)
				if (!isUser) return reply(mess.only.userB)
					...
					buffer = await getBuffer(anu.result)
					dms.sendMessage(from, buffer, video, {mimetype: 'video/mp4', filename: `${anu.title}.mp4`, quoted: mek, caption: 'PWOLIKKEDA MWONUSE'})
					await limitAdd(sender) 	
					break  
